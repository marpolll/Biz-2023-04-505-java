package com.marpol.scanner.models;
 ...
 *                          접근 제한자는 public, protected 가 있는데
 *                          맴버 변수, 맴버 method 에 접근 제한자가 없을때
 *                          
 *                          1. package 가 다른 곳에서 맴버... 에 접근 하려고 하면 접근이 안된다.
 *                          2. public 접근제한자가 있으면 pakage 에 관계없이
...
public class StudentDto {
	 ...
	 *            단, toString() 는 method 소스코드에 나타나지 않는다
	 ...
	 *           효용(필요) 가치가 없다.
	 *           그래서, 원래 toString() method 코드를 감춰버리고
	 *           맴버변수의 값을 문자열로 만들어 return 하는 method 로
	 *           "제 정의(Override)" 했다.
	 */
	@Override
	public String toString() {
		return "StudentDto [stNum=" + stNum + ", stName=" + stName + ", stDept=" + stDept + ", scGrade=" + scGrade
				+ ", stTel=" + stTel + "]";
	}
}